...
# UpSnap settings (space-containing values use upsnap.env)
...
#   health check reports a failure; PocketBase data prevents automatic downgrade.
AUTO_UPDATE=0
...
#   /usr/local/sbin/upsnap-ufw-check                  (service-start firewall guard)
#   /etc/default/ufw, /etc/ufw/ufw.conf               (in-CT IPv4/IPv6 policy)
#   /etc/ufw/user.rules, /etc/ufw/user6.rules         (configured source allows)
#   /opt/upsnap/upsnap.env                         (title and cron values, 0600)
#   /etc/containers/systemd/upsnap.container  (Quadlet unit — source of truth; space-containing values in upsnap.env)
...
# Validate settings before writing the unit and Podman environment file.
...
exec 7>/run/lock/upsnap-creator.lock
flock -n 7 || { echo "ERROR: Another upsnap creator is running." >&2; exit 1; }
...
case "$response" in
  ...
  *)
    ...
    if [[ -f "$SCRIPT_SELF" ]] && head -n 1 "$SCRIPT_SELF" | grep -q '^#!/usr/bin/env bash$'; then
      ...
      [[ ! -e $SCRIPT_LOCAL ]] || SCRIPT_LOCAL="/root/upsnap-quadlet-downloaded.$$.sh"
      DOWNLOAD_TEMP=$(mktemp /root/upsnap-download.XXXXXX)
...
UFWCHECK
pct push "$CT_ID" "$tmp" /usr/local/sbin/upsnap-ufw-check --perms 0755
rm -f -- "$tmp"
pct exec "$CT_ID" -- /usr/local/sbin/upsnap-ufw-check
...
EnvironmentFile=${APP_DIR}/upsnap.env
...
[Service]
ExecStartPre=/usr/local/sbin/upsnap-ufw-check
Restart=always
RestartSec=5
TimeoutStopSec=60
...
# ── UpSnap environment values with spaces ─────────────────────────────────────
{
  printf 'UPSNAP_WEBSITE_TITLE=%s\n' "$UPSNAP_WEBSITE_TITLE"
  printf 'UPSNAP_INTERVAL=%s\n' "$UPSNAP_INTERVAL"
} | pct exec "$CT_ID" -- bash -c 'set -euo pipefail; umask 077; cat > /opt/upsnap/upsnap.env; chmod 0600 /opt/upsnap/upsnap.env'
...
# Quadlet unit whenever the image is updated. Space-containing UpSnap settings live in upsnap.env; other settings live in the unit.
pct exec "$CT_ID" -- bash -lc "
  set -euo pipefail
...
MAIN_SERVICE=upsnap.service
LOCK=/run/lock/upsnap-maint.lock
...
select_component() {
  COMPONENT=$1
  case $COMPONENT in
    APP) CONTAINER=upsnap; KIND=app; IMAGE_RECOVERY=0 ;;
...
valid_tag() {
  case $1 in
    APP) [[ $2 == latest || $2 =~ ^[0-9]+\.[0-9]+\.[0-9]+([.-][A-Za-z0-9._-]+)?$ ]] ;;
    *) return 1 ;;
  esac
}
load_unit() {
  ...
  if (( rc != 0 && SWITCHED )); then
    if (( START_ATTEMPTED == 0 || IMAGE_RECOVERY == 1 )); then
      ...
      if (( restored && APP_STOPPED )); then
        systemctl start "$MAIN_SERVICE" && wait_service upsnap app "${STATE[UPDATE_WAIT_SECONDS]}" || restored=0
      fi
      if (( restored )); then
...
# Semantic version downgrades of persistent components require a separate review.
  ...
  /usr/local/sbin/upsnap-ufw-check || die "Restore active UFW filtering before maintenance."
  ...
  if [[ $COMPONENT != APP ]]; then
    wait_service upsnap app 30 || die "Application is unhealthy before backend update."
  fi
  ...
  WORK=$(mktemp -d /run/upsnap-update.XXXXXX)
  ...
  if [[ $new_id != "$OLD_ID" ]]; then
    ...
    if [[ $COMPONENT != APP ]]; then
      ...
      wait_service upsnap app "${STATE[UPDATE_WAIT_SECONDS]}" || die "Application did not recover after backend update."
    fi
  fi
...
[[ $EUID == 0 ]] || die "Run as root inside the upsnap CT."
...
case $cmd in
  ...
  check)
    ...
    /usr/local/sbin/upsnap-ufw-check
...
MAINT
pct push "$CT_ID" "$tmp" /usr/local/bin/upsnap-maint.sh --perms 0755
rm -f -- "$tmp"

# ── Start via Quadlet ─────────────────────────────────────────────────────────
pct exec "$CT_ID" -- bash -s -- upsnap <<'QUADLET_VALIDATE'
set -euo pipefail
output=$(mktemp)
...
CLEANUP_ON_FAIL=0
pct exec "$CT_ID" -- systemctl start upsnap.service

# Destructive cleanup was disarmed before the first persistent service start.

# ── Verification ──────────────────────────────────────────────────────────────
sleep 30
if ! pct exec "$CT_ID" -- /usr/local/bin/upsnap-maint.sh check --initial; then
...
for key in UPSNAP_WEBSITE_TITLE UPSNAP_INTERVAL; do
  got=$(pct exec "$CT_ID" -- podman exec upsnap printenv "$key") || got=""
  [[ $got == "${!key}" ]] || { echo "ERROR: UpSnap $key env-file round trip failed." >&2; VERIFY_FAIL=1; }
done
unset got
...
[Unit]
Description=upsnap image maintenance
...
[Unit]
Description=upsnap daily image maintenance
[Timer]
OnCalendar=*-*-* $1:00
Persistent=true
[Install]
...
if [[ $AUTO_UPDATE == 1 ]]; then
  pct exec "$CT_ID" -- systemctl enable --now upsnap-update.timer
else
  pct exec "$CT_ID" -- systemctl disable --now upsnap-update.timer
fi
...
cat <<OPERATIONS
  ...
  RUN ON THE PROXMOX HOST
    pct enter $CT_ID
    pct exec $CT_ID -- /usr/local/bin/upsnap-maint.sh version

  RUN INSIDE THE CT
    /usr/local/bin/upsnap-maint.sh check
    /usr/local/bin/upsnap-maint.sh update $APP_TAG
    ufw status verbose
    journalctl -u upsnap.service --no-pager -n 80
