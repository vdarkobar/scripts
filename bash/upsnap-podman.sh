...
TAGS="upsnap;podman;lxc"

# Network scan settings (used in compose environment)
UPSNAP_SCAN_RANGE=""                 # e.g. 192.168.1.0/24 ; blank = no scan range preset
...
APP_TAG="5.3.1"                      # pinned default; do not default to :latest
DEBIAN_VERSION=13

# Optional features / policy
AUTO_UPDATE=0                        # 1 = enable timer-driven maintenance/update runs
TRACK_LATEST=0                       # 1 = auto-update follows ghcr.io/seriousm4x/upsnap:latest
...
#   /opt/upsnap/docker-compose.yml             (Podman compose stack)
#   /opt/upsnap/.env                           (runtime configuration)
#   /opt/upsnap/data/                          (PocketBase persistent data — SQLite DB)
#   /usr/local/bin/upsnap-maint.sh             (maintenance helper)
#   /etc/systemd/system/upsnap-stack.service
...
#   /etc/containers/storage.conf               (Podman storage config)
#   /etc/containers/containers.conf            (Podman container defaults)
...
for cmd in pvesh pveam pct pvesm curl python3 ip awk sort paste readlink cp chmod grep sed seq sleep tail; do
...
[[ "$APP_TAG" == "latest" || "$APP_TAG" =~ ^[0-9]+(\.[0-9]+){0,2}([.-][A-Za-z0-9._-]+)?$ ]] || {
  echo "  ERROR: APP_TAG must be a version like 5, 5.3.1, or latest." >&2
  exit 1
}
[[ -e "/usr/share/zoneinfo/${APP_TZ}" ]] || { echo "  ERROR: APP_TZ not found in /usr/share/zoneinfo: $APP_TZ" >&2; exit 1; }
if [[ -n "$UPSNAP_SCAN_RANGE" ]]; then
  [[ "$UPSNAP_SCAN_RANGE" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+$ ]] \
    || { echo "  ERROR: UPSNAP_SCAN_RANGE must be in CIDR notation (e.g. 192.168.1.0/24): $UPSNAP_SCAN_RANGE" >&2; exit 1; }
fi
...
cat <<EOF2
  ...
  UpSnap-Podman LXC Creator — Configuration
  ────────────────────────────────────────
  CT ID:             $CT_ID
  Hostname:          $HN
  CPU cores:         $CPU
  ...
  App image:         $APP_IMAGE
  App port:          $APP_PORT
  Scan range:        ${UPSNAP_SCAN_RANGE:-(not set)}
  Timezone:          $APP_TZ
  Tags:              $TAGS
...
SCRIPT_URL="https://raw.githubusercontent.com/vdarkobar/scripts/main/bash/upsnap-podman.sh"
SCRIPT_LOCAL="/root/upsnap-podman.sh"
...
while true; do
  ...
  if [[ "$PW1" == *:* ]]; then echo "  Password cannot contain ':'."; continue; fi
...
# ── Set root password inside CT ──────────────────────────────────────────────
# Set via chpasswd over stdin to avoid exposing the password on the host
# process command line (which -password in pct create would do).
...
# UpSnap persistent state: /app/pb_data (PocketBase SQLite DB, user data)
pct exec "$CT_ID" -- bash -lc "
  set -euo pipefail
...
# ── Compose file ──────────────────────────────────────────────────────────────
# UpSnap requires network_mode: host to send Wake-on-LAN magic packets on the
# physical LAN. This means port mapping is not used — the app listens directly
# on APP_PORT. The UPSNAP_HTTP_LISTEN variable controls the bind address.
...
services:
  upsnap:
    ...
    volumes:
      - ${APP_DIR}/data:/app/pb_data:Z
EOF2"

pct exec "$CT_ID" -- bash -lc "
  set -euo pipefail
  cat > '${APP_DIR}/.env' <<EOF2
COMPOSE_PROJECT_NAME=upsnap
...
update_app() {
  ...
  [[ -n "$new_tag" ]] || die "Usage: upsnap-maint.sh update <tag|latest>"
  [[ "$new_tag" == "latest" || "$new_tag" =~ ^[0-9]+(\.[0-9]+){0,2}([.-][A-Za-z0-9._-]+)?$ ]] || die "Invalid tag: $new_tag"
  ...
  rollback() {
    ...
    cd "$APP_DIR"
    /usr/bin/podman-compose up -d --force-recreate upsnap || true
  }
  trap rollback ERR
  ...
  echo "  Recreating UpSnap container ..."
  cd "$APP_DIR"
  /usr/bin/podman-compose up -d --force-recreate upsnap

  echo "  Waiting for UI ..."
  if ! wait_for_app; then
    trap - ERR
    rollback
    die "UpSnap did not become reachable after update."
  fi
...
pct exec "$CT_ID" -- bash -lc '
  set -euo pipefail
  cat > /etc/systemd/system/upsnap-stack.service <<EOF2
[Unit]
Description=UpSnap (Podman) stack
...
EOF2
  ...
  systemctl enable --now upsnap-stack.service
'
...
if pct exec "$CT_ID" -- systemctl is-active --quiet upsnap-stack.service 2>/dev/null; then
  echo "  UpSnap stack service is active"
else
  echo "  ERROR: upsnap-stack.service is not active" >&2
  echo "  Check: pct exec $CT_ID -- journalctl -u upsnap-stack.service --no-pager -n 50" >&2
  VERIFY_FAIL=1
fi
...
pct exec "$CT_ID" -- bash -lc 'cd /opt/upsnap && podman-compose ps' || true

if [[ "$RUNNING" -lt 1 ]]; then
  echo "  ERROR: No Podman containers running" >&2
  echo "  Check: pct exec $CT_ID -- bash -lc 'cd /opt/upsnap && podman-compose logs --tail=80'" >&2
  VERIFY_FAIL=1
fi
...
if [[ "$US_HEALTHY" -eq 1 ]]; then
  ...
  echo "  ERROR: UpSnap not responding on port ${APP_PORT}" >&2
  echo "  Check: pct exec $CT_ID -- bash -lc 'cd /opt/upsnap && podman-compose logs --tail=80'" >&2
  VERIFY_FAIL=1
fi
...
port=\"\${port:-8090}\"
printf '  Stack:     /opt/upsnap (%s containers running)\n' \"\$running\"
printf '  Compose:   cd /opt/upsnap && podman-compose [up -d|down|logs|ps]\n'
printf '  Maintain:  upsnap-maint.sh [update|auto-update|version]\n'
...
<details><summary>Details</summary>UpSnap (Podman) — Wake-on-LAN web app on Debian ${DEBIAN_VERSION} LXC
Created by upsnap-podman.sh</details>"
...
echo "  Config files:"
echo "    /opt/upsnap/.env"
echo "    /opt/upsnap/docker-compose.yml"
echo ""
echo "  Maintenance:"
echo "    pct exec $CT_ID -- /usr/local/bin/upsnap-maint.sh update <tag|latest>"
echo "    pct exec $CT_ID -- /usr/local/bin/upsnap-maint.sh auto-update"
...
echo ""
echo "  Notes:"
echo "    UpSnap uses network_mode: host for WoL magic packets."
echo "    First visit at http://${CT_IP}:${APP_PORT}/ creates the admin account."
echo "    Do not expose UpSnap publicly — use a VPN for remote access."
echo ""
